<h1 align="center">Eduardo L. França</h1>

<p align="center">
  Desenvolvedor Full Stack
</p>

<p align="center">
  Desenvolvedor com experiência em aplicações web, APIs REST, automações,
  banco de dados e soluções com inteligência artificial.
</p>

<p align="center">
  Atuo principalmente com Python, Django, React, PostgreSQL e Docker,
  desenvolvendo sistemas escaláveis e focados em resolver problemas reais.
</p>

---

## Sobre mim

- Desenvolvedor Full Stack no Instituto de Tecnologia Anexo Brasil
- Atuação nos projetos SADI - Educação e SADI - Online, desenvolvendo aplicações web, APIs REST, integrações e soluções com inteligência artificial
- Experiência com visão computacional, inteligência artificial e modelos de detecção
- Arquitetura de software, código limpo e boas práticas
- Goiânia - GO, Brasil

---

## Tecnologias e Ferramentas

<p align="left">
  <img alt="Linux" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" />
  <img alt="Python" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" />
  <img alt="JavaScript" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" />
  <img alt="Django" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" />
  <img alt="React" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" />
  <img alt="PostgreSQL" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" />
  <img alt="MySQL" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" />
  <img alt="Docker" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" />
  <img alt="Git" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" />
  <img alt="GitHub" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" />
  <img alt="Figma" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" />
  <img alt="Photoshop" height="40" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/photoshop/photoshop-plain.svg" />
</p>

<p>
  <strong>Também trabalho com:</strong> Django REST Framework, Machine Learning, Inteligência Artificial, YOLOv11, Excel e automações.
</p>

---

## Projetos em Destaque

### [Incident-Atlas](https://github.com/EduFrancaDev/Incident-Atlas)
Sistema para análise e visualização de acidentes industriais, utilizando dados e visualizações para apoiar a tomada de decisão.

### [AIR-MOUSE---CONTROLE-POR-GESTOS](https://github.com/EduFrancaDev/AIR-MOUSE---CONTROLE-POR-GESTOS)
Aplicação em Python com visão computacional para controle do computador por gestos.

### [Bot-Whatsapp](https://github.com/EduFrancaDev/Bot-Whatsapp)
Automação em Python e Selenium para envio de mensagens no WhatsApp Web a partir de planilhas.

### [Projeto-ELA-Android](https://github.com/EduFrancaDev/Projeto-ELA-Android)
Aplicativo Android voltado para apoio e segurança de mulheres em situação de risco.

---

## Estatísticas

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=EduFrancaDev&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=EduFrancaDev&layout=compact&theme=tokyonight" />
</p>

<p align="center">
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=EduFrancaDev&theme=tokyonight" />
</p>

---

## Contato

<p align="left">
  <a href="https://www.linkedin.com/in/eduardo8franca/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Eduardo%20Fran%C3%A7a-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>
